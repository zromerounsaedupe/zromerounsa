...
// Este conjunto de controles realiza la órbita, la plataforma rodante (zoom) y la panorámica.
// A diferencia de TrackballControls, mantiene el "up" direccional object.up (+Y por defecto).
//
...
//control de la camara de inicio
const controls = new OrbitControls(camera, renderer.domElement);
controls.maxPolarAngle = Math.PI * 0.5;
controls.target = new Vector3(0, 30, -50);
...
// Add the sun
light.add(
    new Mesh(new SphereGeometry(sceneRadius/5, 36, 36), new MeshBasicMaterial({
        color: 0xffaa33
    }))
);
...
const floor = new Mesh(
    new PlaneGeometry(sceneRadius*5,sceneRadius*5,100,100),
	...
	if (session && frame) {
		...
		if (window.__dumpHands) {
			...
			if (hands.left && hands.right) {
				...
				outData[0] = size;
				const leftHandAccessor = new Float32Array(outData.buffer, 4, size * 50);
				const rightHandAccessor = new Float32Array(outData.buffer, 4 + (size * 16 * 4), size * 50);
				const weights = new Float32Array(outData.buffer, 4 + 2 * (size * 16 * 4), size * 50);
				weights.fill(1);
...
(async function () {
    ...
    const {scene: gltfScene} = await new Promise(resolve => loader.load('./assets/casita.glb', resolve));
    const house = gltfScene.children[0];
    house.position.x = -80;
	house.position.z = 110;
    house.position.y = 1;
